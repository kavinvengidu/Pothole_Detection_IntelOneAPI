...
model = tf.keras.models.load_model("D:\pothole\densenetmodel.h5")

# Define the safety gear class labels
safety_gear_labels = ["0","1","2"]
...
def app():
    st.title("Pothole")
    st.write("Upload an image to detect pothole objects.")

    # Sidebar for selecting input type
    input_type = st.sidebar.selectbox("Select Input Type", options=["Image"])
